...
| **Esc** | **关闭窗口 / 菜单** | 如果有上下文菜单打开则优先关闭菜单，否则关闭搜索窗。 |
| **↑ / ↓** | **选择结果** | 在搜索结果列表中上下移动选中项。 |
| **Enter (回车)** | **执行 / 打开** | 打开选中的文件，或执行选中的命令。 |
| **Tab** | **自动补全** | 将当前选中项的文本填入搜索框（例如补全命令路径）。 |

### 进阶操作
| 按键 | 作用 | 说明 |
| :--- | :--- | :--- |
| **Shift + ↑** | **显示历史记录** | **仅在搜索框为空时有效**。显示最近使用的命令和收藏项。 |
| **Alt + F** | **收藏/取消收藏** | 将当前选中的命令加入或移出“收藏夹”（Favorites）。 |
| **Shift + →** | **打开上下文菜单** | 等同于鼠标右键点击选中项，打开操作菜单。 |
| **Shift + Enter** | **执行 (Shift模式)** | 部分命令在按住 Shift 执行时会有不同行为（如快速编辑而不关闭窗口等，视具体实现而定）。 |
